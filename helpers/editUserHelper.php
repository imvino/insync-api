...
<?php
	...
	case "addUser":
	{
		...
		if($result = pg_query_params($db, "INSERT INTO users (id,name,pass,salt,digest_password) VALUES ($1,$2,$3,$4,$5)",
                        array($new_uuid, $username, $hash, $salt, $digestHash)))
		{
			if($result == FALSE)
			{
        ...
        if (update_user_permissions_by_name($db, $username, $permissions)) 
        {
            ...
            if($password != "")
            {
                if($result = pg_query_params($db, "UPDATE users SET pass=$1,salt=$2,digest_password=$3 WHERE name=$4",
                            array($hash, $salt, $digestHash, $username)
                        ))
                {
                    if($result == FALSE)
                    {
	...
	case "viewUser":
	{
		...
		if($result = pg_query_params($db, "SELECT u.name as name, p.name as permission FROM users u right outer join permissions p on u.id = p.user_id WHERE u.name=$1", array($name)))
		{
            ...
            if($result = pg_query_params($db, "SELECT s.name as key, s.value as value FROM users u right outer join user_settings s on u.id = s.user_id WHERE u.name=$1", array($name)))
            {
                while ($resultRow = pg_fetch_assoc($result))
                {
	...
	case "deleteUser":
	{
		...
		if($result = pg_query_params($db, "DELETE FROM users WHERE name=$1", array($username)))
		{
			if($result == FALSE)
			{
                                pg_close($db);
	...
	case "sync":
	{
        ...
        if(!$receive)
        {
			...
			if ($Intersections)
			{
	            ...
	            $postParams = array("action"=>"sync", "receive"=>"true", "data"=>$data, "u" => base64_encode("PEC"), "p" => base64_encode("lenpec4321"));
...
function updateUserTables($data)
{
    ...
    foreach($xmlDoc->usertable->row as $user)
    {
        ...
        if(!pg_query_params($db, "INSERT INTO users (id,name,pass,salt,digest_password,hidden) VALUES ($1,$2,$3,$4,$5,$6)",
                        array($id, $name, $pass, $salt, $digest_password, $hidden)
                ))
        {
            pg_query($db, "ROLLBACK TRANSACTION");
            pg_close($db);
...
{
    if (pg_query_params($db, 'delete from user_settings where user_id = $1 and name = $2', array($user_id, $setting))) 
    {
        if (!pg_query_params($db, 'insert into user_settings (user_id, name, value) values ($1, $2, $3)',
                array($user_id, $setting, $value)))
...
{
    if (pg_query_params($db, 'delete from user_settings where user_id in (select id from users where name = $1) and name = $2', array($user_name, $setting))) 
    {
        if (!pg_query_params($db, "insert into user_settings (user_id, name, value) select id, $2, $3 from users where name = $1",
                    array($user_name, $setting, $value)))
            die("Error: Failed to add new setting." . pg_last_error($db));
...
function update_user_permissions_by_id($db, $user_id, $permissions) {
    if (pg_query_params($db, 'delete from permissions where user_id = $1', array($user_id))) {
